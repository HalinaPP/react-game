...
const backtrackBased = (initBoard: number[][]): number[][] | boolean => {
  let board = [...initBoard.map(row => [...row])];
  console.log('=0' );
  for (let i = 0; i < size; i++) {
    for (let j = 0; j < size; j++) {
      console.log('board value' +board[i][j]+ 'i='+i+' j='+j);
      if (board[i][j] === 0) {
        console.log('=0' + 'i='+i+' j='+j);
        completeCell(board, i, j);
        const afterComplete = [...board.map(row => [...row])];
        ...
        if (cellValue.length > 1) {
          console.log('cell array',cellValue);
          for (let k = 0; k < cellValue.length; k++) {
            let boardCopy = [...board.map(row => [...row])];

            boardCopy[i][j] = cellValue[k];

            const completed_board = backtrackBased(boardCopy);
            console.log('after completed board',completed_board );
            if (completed_board) {
...
export const solve = (board3: number[][]) => {
  ...
  /*if (!solved) {
    const newBoard = backtrackBased(board4);
    console.log('neeeeewwww board=',newBoard);
    board2 = newBoard ? newBoard : board4;
    solved = isSolved(board2);
  }*/
